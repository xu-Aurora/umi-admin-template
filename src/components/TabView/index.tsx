...
const TabView: FC<Props> = (props) => {
  const { tabs, setTabs } = useModel('tabPanes')
  const { reloadPath, curTab } = tabs

  // const curTab = tabs.curTab
  // const reloadPath = tabs.reloadPath

  const [activeKey, setActiveKey] = useState<string>('')
  const [panes, setPanes] = useState<CommonObjectType[]>([
    {
      title: '首页',
      key: 'home',
      content: Home,
  ...
  const removeAll = async (isCloseAll?: boolean) => {
    ...
    history.push(isCloseAll ? '/home' : path)
        ...
        {panes.map((pane: CommonObjectType) => (
          ...
          >
            {reloadPath !== pane.path ? (
              ...
              <div style={{height: '50vh'}} className='flex_c'>
                <Spin tip="刷新中..." size='large' />
              </div>
            )}
          </TabPane>
        ))}
